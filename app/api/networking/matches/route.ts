...
import { getNetworkingMatches, isValidUuid } from "@/lib/networking/service";
...
function getSafeErrorMessage(error: unknown) {
  ...
  if (process.env.NODE_ENV === "production") {
    return "Eslesmeler alinamadi.";
  }

  return message || "Eslesmeler alinamadi.";
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    const matches = await getNetworkingMatches(profileId);
    if (!matches) {
